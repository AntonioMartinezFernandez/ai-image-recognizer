...
func main() {
	...
	flag.StringVar(&folder, "folder", "assets/recognizer", "Folder containing images to process")
	...
	if category == "" {
		fmt.Printf("No category provided. Please use the category flag to specify a category. e.g., make recognizer category=fruit\n")
		os.Exit(1)
	}
...
func processImage(assetsFolder, jpgFile string, category string, llamaCppClient *llamacpp.Client, wg *sync.WaitGroup) {
	...
	response, err := llamaCppClient.RecognizeSubject(category, *base64Image)
	if err != nil {
